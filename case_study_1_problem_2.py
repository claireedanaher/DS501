...
def top30words(json_tweet_data):
    # gathers the text from each tweet into a list
    all_tweets = []
...
def top_retweeted(json_tweet_data):
    # gathers number of retweet counts for each tweet into a list
    retweet_counts = []
    for tweet in json_tweet_data:
        retweet_counts.append(tweet['retweet_count'])

    # finds indices of top 10 retweet counts
    top10indices = sorted(range(len(retweet_counts)), key = lambda i: retweet_counts[i])[-10:]
    top10indices.reverse()
